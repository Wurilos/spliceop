...
// Memoized table row component - using any for memo compatibility
const TableRowMemo = memo(function TableRowMemo({
  ...
  customActions?: (row: { id: string }) => React.ReactNode;
  getNestedValue: (row: { id: string }, key: string) => unknown;
}) {
...
export function LazyDataTable<T extends { id: string }>({
  ...
  const getNestedValue = useCallback((row: T, key: string): unknown => {
  ...
  }, [data, search, searchKey, getNestedValue]);
  ...
  const { allPageSelected, somePageSelected } = useMemo(() => {
    const all = paginatedData.length > 0 && paginatedData.every(row => selectedIds.has(row.id));
    const some = paginatedData.some(row => selectedIds.has(row.id));
  ...
  }, [onDeleteMany, selectedIds, clearSelection]);
  ...
  }, [onDeleteMany, filteredData, clearSelection]);

  const handleDeleteRow = useCallback((row: T) => {
    setRowToDelete(row);
  }, []);
          ...
          <TableBody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row) => (
                <TableRowMemo
                  key={row.id}
                  row={row as { id: string; [key: string]: unknown }}
                  columns={columns as Column<{ id: string }>[]}
                  selectedIds={selectedIds}
                  onSelectRow={handleSelectRow}
                  onEdit={onEdit as ((row: { id: string }) => void) | undefined}
                  onDelete={handleDeleteRow as ((row: { id: string }) => void) | undefined}
                  customActions={customActions as ((row: { id: string }) => React.ReactNode) | undefined}
                  getNestedValue={getNestedValue as (row: { id: string }, key: string) => unknown}
                />
              ))
            )}
          </TableBody>
